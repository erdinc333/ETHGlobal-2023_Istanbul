...
export class UserTicketsUpdated extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save UserTicketsUpdated entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type UserTicketsUpdated must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("UserTicketsUpdated", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): UserTicketsUpdated | null {
    return changetype<UserTicketsUpdated | null>(
      store.get_in_block("UserTicketsUpdated", id.toHexString())
    );
  }

  static load(id: Bytes): UserTicketsUpdated | null {
    return changetype<UserTicketsUpdated | null>(
      store.get("UserTicketsUpdated", id.toHexString())
    );
  }
  ...
  get eventIdOfTicket(): BigInt {
    let value = this.get("eventIdOfTicket");
    if (!value || value.kind == ValueKind.NULL) {
      throw new Error("Cannot return null for a required field.");
    } else {
      return value.toBigInt();
  ...
  set eventIdOfTicket(value: BigInt) {
    this.set("eventIdOfTicket", Value.fromBigInt(value));
  }
